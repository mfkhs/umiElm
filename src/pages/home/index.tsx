  ...
  return (
    <div>
      ...
      <NavBar className={styles.header_top}
        ...
        rightContent={[
          <span key='0' onClick={()=>history.push('/login')
        }>登录|注册</span>
        ]}>
      </NavBar>
      {/* 城市选择提示 */}
      ...
      <div className={styles.select_city}> 
        ...
        <section className={styles.hot_city_container}>
            ...
            <ul className={styles.citylistul}>
              {
                data.hotcity.map((item:hotcityItem, index:number) => {
                  return (<li  key={`${index}` + item.area_code}>{item.name}</li>)
                })
              }
            </ul>
        </section>
              ...
              Object.keys(sortgroupcity(data.groupcity)).map((itemkey:string, index:number) => {
                return (
                    <li className={styles.letter_classify_li} key={itemkey + index}>
                      ...
                      <ul className={styles.citylistul}>
                        {
                          sortgroupcity(data.groupcity)[itemkey].map((item: hotcityItem, index: number) => {
                            return (
                            <li className={`ellipsis ${styles.title}`} key={`${index}${item.id}`} 
                            onClick={()=>history.push({pathname:'/city', query:{id: item.id}})}
                            >{item.name}</li>
                            )
                          })
                        }
